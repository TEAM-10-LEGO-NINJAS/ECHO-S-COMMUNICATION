...
while True:
    _,frame=cap.read()
    count = {
             'a': len(os.listdir(directory+"/accident")),
    ...
    row = frame.shape[1]
    col = frame.shape[0]
    cv2.rectangle(frame,(100,120),(600,450),(0,0,0),4)
    cv2.imshow("data",frame)
    cv2.imshow("ROI",frame[10:400,250:350])
    interrupt = cv2.waitKey(10)
    if interrupt & 0xFF == ord('a'):
        cv2.imwrite(directory+'accident/'+str(count['a'])+'.png',frame)
